...
#include "inference_basic_sample_app.hpp"

#if CONFIG_INFERENCE_BASIC_SAMPLE_APP
...
#define DELAY_BETWEEN_INFERENCES_MS	(2000)
...
#if CONFIG_MODULE_ARM_NPU_ENABLED
    #define ARENA_SIZE  (648000)
#else
    #define ARENA_SIZE  (200000)
#endif
static uint8_t tensor_arena_1[ARENA_SIZE] ACTIVATION_BUF_ATTRIBUTE;
...
void single_model_sample_app(void *argument)
{
	...
	if (!infer_job::setup())
	{
		LOG_ERROR(LOG_MOD_INFERENCE, "Error setting up infer job\n");
	}

	int num_of_inferences = 3;
	const uint8_t* model_ptr = model::get_model_pointer();
	// Declare a inference job object
	infer_job job;

	// Initialize the model with the tflite location, tensor arena area and resolver
	LOG_INFO(LOG_MOD_INFERENCE, "Loading model from memory\n");
	if (!job.init(model_ptr, tensor_arena_1, sizeof(tensor_arena_1), model::get_resolver()))
	{
	...
	// Create semaphore used for signaling the application task from inference call back
	...
	for (int k = 0; k< num_of_inferences; k++)
	{
		...
		{
			job.set_input_data(i,model::get_user_input_buffer(i));
		}
		...
		// Example how to access output scale and zero point of output[0]		
		TfLiteTensor* tensor = job.get_output_tensor(0);		
		LOG_INFO(LOG_MOD_INFERENCE, "output[0] quantization scale=%f, zero_point=%d\n",tensor->params.scale, tensor->params.zero_point);

		// Dump outputs of the model and compare each result with the expected output
		LOG_INFO(LOG_MOD_INFERENCE, "Comparing inference results vs expected results from Python\n");
		LOG_INFO(LOG_MOD_INFERENCE, "This model has %d output tensors\n", job.get_num_of_outputs());
		for(unsigned int i=0; i < job.get_num_of_outputs(); i++)
		{
			...
			job.print_error_metrics(i, model::get_expected_output_buffer(i));
		}

		// Wait for some time
		LOG_INFO(LOG_MOD_INFERENCE, "Delay for %d ms\n", DELAY_BETWEEN_INFERENCES_MS);
		vTaskDelay(DELAY_BETWEEN_INFERENCES_MS);
	} // for
...
#endif // CONFIG_INFERENCE_BASIC_SAMPLE_APP
