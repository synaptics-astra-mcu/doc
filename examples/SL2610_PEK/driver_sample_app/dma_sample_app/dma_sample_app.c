...
void dma_all_transfers() {
    ...
    if ((rc = dma_single_transfers_2d_copy()) != DMA_OK) {
        LOG_ERROR(LOG_MOD_DMA, "2D DMA transfer failed.\n");
        err_count++;
    }
...
#if DMA_TRANSFER_2D_COPY
    LOG_INFO(LOG_MOD_DMA, "Running 2D DMA Transfer Test...\n");
    rc = dma_single_transfers_2d_copy();
    if (rc != DMA_OK) {
        LOG_ERROR(LOG_MOD_DMA, "2D transfer test failed.\n");
    }
#endif
