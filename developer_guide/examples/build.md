# 🔧 Astra MCU SDK Examples - Build Guide

Complete step-by-step guide for building Synaptics SDK (Astra MCU SDK) and example applications.
...
├── SR110_RDK/                      # RDK-specific sample apps
...
├── kconfig.advanced                # Example configurations
├── docs/
...
- **subdirectories.cmake:** Located at `examples/tools/cmake/subdirectories.cmake`, this file lists which example subdirectories to include in the build (e.g., `add_subdirectory(SR110_RDK)`). It's included by CMakeLists.txt and allows you to easily control which examples are built. You can add or remove entries to customize the build scope.
- **kconfig.advanced:** Located at `examples/kconfig.advanced`, this is used so that the SDK’s main Kconfig can source this file and include the application’s configs during an SDK + Example combined build.
## Prerequisites:
...
ls install/SR110_RDK/config/*.ld  # For GCC
ls install/SR110_RDK/config/*.sct # For AC6
...
# Build multiple applications using same SDK
...
make cm55_demo_sample_app_defconfig BOARD=SR110_RDK
make build BOARD=SR110_RDK
make cm55_audio_example_defconfig BOARD=SR110_RDK
make build BOARD=SR110_RDK
```
...
> **Debug Build Prerequisite:** Build the TensorFlow Lite (tflite) library in Debug mode inside the Astra MCU SDK directory before building the Debug executable.
